...
fn main() {
    ...
    for file in files {
        ...
        let new_file_name = if br_args.use_new_name {
            ...
            let file_name = file_name.replace(br_args.old.as_ref().unwrap().as_str(), br_args.new.as_ref().unwrap().as_str());
            let x: Vec<&str> = file_name.split(".").collect();
            format!("{}{}{}.{}", br_args.prefix.as_ref().unwrap_or(&String::from("")), x[0], br_args.suffix.as_ref().unwrap_or(&String::from("")), x[1])
        } else {
            let x: Vec<&str> = file_name.split(".").collect();
            format!("{}{}{}.{}", br_args.prefix.as_ref().unwrap_or(&String::from("")), x[0], br_args.suffix.as_ref().unwrap_or(&String::from("")), x[1])
        };
